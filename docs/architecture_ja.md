...
モデル 3 種が揃った Standalone セッションでは、ワーカーが RVC とパススルーの
両経路を保持し、入力チャンクごとにライブパススルーフラグを確認します。
パススルー中は RVC 推論を呼ばず、入力ゲイン、選択中の入力デノイザ、
デバイスレートへのリサンプリング、出力ゲインを順に適用します。RVC に戻る際は、
次のチャンクを処理する前に古い RVC ローリング文脈とスムーザー履歴を破棄します。
モデルなしセッションではパススルー経路だけを使います。
