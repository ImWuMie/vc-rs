...
1. **Models** — **Browse** から RVC モデル・埋め込み（ContentVec）・F0（RMVPE）
   の各 `.onnx` を指定します。
2. **Model pool (live switch)** — **Add model…** で追加の RVC モデルを
   リアルタイム切替プールへ登録します（この時点では読み込まれません）。
   **Switch** を押すと対象モデルを**バックグラウンドで読み込み**、完了後に自動で
   アクティブにします（リストに読み込み進捗が表示）。読み込み済みのモデルは
   再起動なしに即時切替できます。プール一覧は永続化されますが、モデルは
   **遅延読み込み**（Switch したときだけ読み込み）です。各行の **✕** でプール
   から削除できます。
3. **Provider** — バックエンドを選びます（windowsml 版: `windowsml` /
   `windowsml-directml` / `windowsml-nvtrtx` / `windowsml-cpu` / `cpu`、
   tensorrt 版: `tensorrt`）。**GPU Priority** と CUDA / TensorRT 用の
   **GPU Device ID** も選べます。
4. **Audio** — 入力・出力デバイスを選びます（**Refresh devices** で再取得）。
   空欄なら「System default」を使います。**Enable monitor output** をオンにすると
   2 つ目の出力デバイス（例: ヘッドホン）で変換後の信号を同時に再生でき、
   **Monitor device** を選びます。実行中のデバイス変更は**リアルタイムで反映**:
   サンプルレートが同じ切替は即時（セッションは再起動されません）、サンプルレート
   が変わると自動的にセッションが再起動します。
5. **Engine configuration (Apply to restart)** — **Chunk ms** / **Extra convert
   ms** を設定します（下記「リアルタイム設定の調整」）。これらの変更は、ベース
   モデルのパスや Provider と同様、**Apply / Start** を押すまで適用されません。
6. **Apply / Start** を押すと反映・開始します。**Stop** で停止します。
7. **Live parameters**（Pitch shift / Speaker ID / Input gain / Output gain /
   Monitor gain）はリアルタイムに反映されます。
8. **Input denoiser** — `off` / `noise-gate` / `rnnoise` / `gtcrn` の間で
   **実行中にリアルタイムで切り替え**られます（後述）。noise-gate のしきい値なども
   リアルタイム反映されます。
9. **Telemetry** に推論時間・入出力 RMS・オーバーラン/アンダーランが表示され、
...
入力ノイズ抑制は **Input denoiser** で `off` / `noise-gate` / `rnnoise` /
`gtcrn` を**実行中にリアルタイムで切り替え**られます: 最初の 3 つはバックグラウンド
で即座に構築・ホットスワップされ、GTCRN は初回切替時にエンジンをバックグラウンド
で読み込み（読み込み中は以前のノイズ抑制を継続）、完了後に自動でホットスワップ
されます。パススルーにも Input gain、選択中の入力ノイズ抑制、Output gain が適用
されます。RNNoise は組み込みモデルを使うため追加モデルは不要です。VST3 版には
これらの入力ノイズ抑制は含まれません。
